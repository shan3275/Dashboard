...
class Chart{
  ...
  */
   ...
   * 设置柱状图
   */
  static function createcolumnar($title,$data=array(),$data2=array(),$size=40,$height=100,$width=80,$legend=array()){
  	    //载入文件
		vendor("Jpgraph.jpgraph");
		...
		$graph->img->SetMargin(100,30,40,100);
  ...
  */
   ...
   * 线图
   */
  function createmonthline($title,$data=array(),$data2=array(),$size=40,$height=100,$width=80,$legend=array()){
    vendor("Jpgraph.jpgraph");
   ...
   * 设置多柱状图
   */
    function createcolumnar2($title,$data=array(),$name=array(),$size=40,$height=100,$width=80,$legend=array()){
  	    //载入文件
		vendor("Jpgraph.jpgraph");
		vendor("Jpgraph.jpgraph_bar");

		// Some data
		// $datay1=array(5001,110,50,60);
		...
		$graph = new \Graph($width,$height,"auto");
		$graph->SetScale("textlin");
		$graph->img->SetMargin(120,150,30,100);
		...
		$graph->yaxis->title->Set(iconv("UTF-8","GB2312//IGNORE","￥"));
		...
		foreach($data as $key => $val){
			...
			// 
			$$bplot->SetFillColor("#".$key.$key.$key."@0.4");
			// $$bplot->SetFillColor('#000@0.4');
			// Setup legends
			$$bplot->SetLegend(iconv("utf-8","gb2312",$name[$key]));
